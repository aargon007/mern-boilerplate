...
const auth = (...requiredRoles: TUserRole[]) => {
    return catchAsync(async (req: Request, res: Response, next: NextFunction) => {
        ...
        if (!user) {
            throw new AppError(httpStatus.NOT_FOUND, "User not found!", "Please provide valid token!");
        }
        ...
        if (requiredRoles && !requiredRoles.includes(role)) {
            throw new AppError(httpStatus.UNAUTHORIZED, "You are not authorized!", "You do not have the necessary permissions to access this resource.");
        }
