...
const changePassword = async (
    ...
    if (
        ...
        user.passwordHistory.slice(0, 2).some((prevPassword) =>
            bcrypt.compareSync(newPassword, prevPassword.password)
        )
    ) {
        const lastUsedPasswordDate = user.passwordHistory[0].updatedAt.toLocaleString();
        throw new AppError(
            httpStatus.BAD_REQUEST,
    ...
    const passwordHistoryItem = { password: user.password, updatedAt: user.passwordChangedAt || new Date() }
